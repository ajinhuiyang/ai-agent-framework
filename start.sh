...
#
# 用法:
#   ./start.sh                    # 默认使用 14B 模型
#   ./start.sh --model 7b         # 使用 7B 模型
#   ./start.sh --model 1.5b       # 使用 1.5B 模型 (最快)
#   ./start.sh --stop             # 停止所有服务
#   ./start.sh --status           # 查看服务状态
#   ./start.sh --test             # 启动后发送测试请求
#
...
get_model_file() {
    case "$1" in
        22b|codestral) echo "Codestral-22B-v0.1-Q5_K_M.gguf" ;;
...
VALID_MODELS="22b 14b 7b 1.5b codestral"
DEFAULT_MODEL="22b"
...
stop_all() {
    ...
    # 兜底: go run 会 fork 子进程, PID 文件可能记录的是父进程 PID,
    # 杀父进程后子进程可能仍占用端口。按端口强杀残留进程。
    for port in $PORT_NLU $PORT_LLM_GEN $PORT_RAG $PORT_LLM_ENGINE; do
        local pids
        pids=$(lsof -ti ":$port" 2>/dev/null || true)
        if [[ -n "$pids" ]]; then
            log_warn "端口 $port 仍被占用, 强制清理 (PIDs: $pids)"
...
check_and_build() {
    ...
    # 检查 C++ 二进制
    local cpp_bin="$ROOT_DIR/Local-LLM/build/localllm"
    if [[ ! -f "$cpp_bin" ]]; then
        log_step "构建 C++ 推理引擎"
        mkdir -p "$ROOT_DIR/Local-LLM/build"
        (cd "$ROOT_DIR/Local-LLM/build" && cmake -DCMAKE_BUILD_TYPE=Release .. && make -j"$(sysctl -n hw.ncpu)")
        log_ok "C++ 引擎构建完成"
    fi
...
start_rag() {
    ...
    (cd "$ROOT_DIR/RAG" && go run cmd/server/main.go) \
        > "$LOG_DIR/rag.log" 2>&1 &
    save_pid "rag" $!
...
start_llm_generation() {
    ...
    (cd "$ROOT_DIR/LLM-Generation" && go run cmd/server/main.go) \
...
start_nlu() {
    ...
    (cd "$ROOT_DIR/NLU" && go run cmd/server/main.go) \
        > "$LOG_DIR/nlu.log" 2>&1 &
    save_pid "nlu" $!
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                ...
                echo ""
                echo "示例:"
                echo "  $0 --14b              # 启动 14B 模型全链路"
                echo "  $0 --1.5b --test      # 启动 1.5B 并发送测试"
                echo "  $0 --stop             # 停止所有服务"
                exit 0
                ;;
            *)
